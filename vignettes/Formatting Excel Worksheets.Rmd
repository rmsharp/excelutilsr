...
get_earned_pts <- function(grade_df) {
  ...
  t_df <- cbind(grade_df[ , student_col], current_grade_df)
  t_df
}
get_current_avg <- function(df) {
  student_col <- get_student_col(df)
  possible_points <- get_possible_pts(df)
  earned_points <- get_earned_pts(df) # needs to be cumulative earned points
  current_avg <- earned_points[ , -student_col] / possible_points[ , -student_col]
  current_avg <- cbind(earned_points[ , student_col], current_avg)
  current_avg
}
...
full_grade_df <- function(Student = c("James", "Jane", "Jamie"), 
                      )
grade_df <- data.frame(Student = c("James", "Jane", "Jamie"), 
                      "Exam_1" = c(86, 35, 42),
                      "Exercise_1" = c(75, 0, 95),
...
add_student_col <- function(current_avg, score_test_col, student_col) {
  ...
  for (col in seq_along(current_avg)) {
    if (col != student_col) {
      student_df <- cbind(student_df, false_col)
    } else {
      student_df <-cbind(student_df, student = score_test_col)
    }
  }
